...
2. Hard reload through the installed `agent-browser` native request-header and reload commands. While attached only to the supervisor-provided same origin, run `agent-browser --session <name> set headers '{"Cache-Control":"no-cache","Pragma":"no-cache"}'`, require command success, then run `agent-browser --session <name> reload`. The HTTP request directives require caches to revalidate rather than reuse a response without validation, matching the enforceable cache-bypass/revalidation property needed by this procedure. Wait for a semantic control or authoritative visible state to reconstruct. In the new `hard_reload` segment require exactly one new document and dev-ticket request, and inspect their `agent-browser network request <request-id>` details to retain only the safe typed facts that both sent `Cache-Control: no-cache` and `Pragma: no-cache`. Command success without the new navigation, exact traffic, request-header proof, and observable reconstruction fails. Immediately run `agent-browser --session <name> set headers '{}'`, require cleanup command success, and record that the temporary headers are no longer active. If header application, request-detail evidence, reload navigation, reconstruction, or cleanup cannot be evidenced, record a specific capability blocker and fail the attempt. Do not substitute `Meta+Shift+R`, `location.reload(true)`, CacheStorage/service-worker clearing, an external CDP client, or an alternate browser harness.
