...
4. Fresh context: complete the disposal procedure, close only the named session, start a second globally unique session with a distinct fresh ephemeral profile, and before its explicit login flow prove zero prior browser-cache entries, zero IndexedDB databases, zero session-storage entries, no loaded persistent state, and a null prior-context nonce. Record the second session name, prove it differs from the initial name, then exercise only the required login flow and repeat identity/stale-context proof and the critical visible workflow. The `fresh_context` segment independently requires one document, one dev-ticket request, and observable reconstruction. Fresh empty-context evidence is separate from ordinary reload and is not hard-reload proof.
