...
Retain the complete raw HTTP request sequence across three explicit capture segments in this exact order: `initial_load`, `ordinary_reload`, and `fresh_context`. Start each segment before its named browser trigger and end it only after the semantic/observable state used by that reconstruction step is reached. `network.json` uses `gooseweb-network-capture/v4`: its ordered segment descriptors record the exact trigger, context generation, completeness bounds, and derived raw-request count, and every raw HTTP entry carries exactly one `segment_id`. Do not clear, replace, filter, or restart the accumulated capture between segments. The initial and ordinary-reload segments belong to context generation 1; the second unique ephemeral session/profile is context generation 2.
...
1. Ordinary reload: run exactly `agent-browser --session <name> reload`, require command success, and wait for an observable control/state rather than a fixed sleep. The `ordinary_reload` segment must contain exactly one newly observed document request and one dev-ticket request and must reach semantic reconstruction with the measured missing/duplicate/order counts. This is ordinary reload only. Do not apply `Cache-Control: no-cache` or `Pragma: no-cache`, and never describe the command or its traffic as cache-bypassing, DevTools hard reload, or equivalent proof.
2. Navigation away/back: use visible top-navigation controls to leave Agents, then a visible control to return; browser `back` alone is insufficient if the product navigation is an in-app view switch.
3. Reconnect: use the supervisor-provided one-layer fault control/observer. The reviewer does not restart the stack. Verify honest offline/stale/replaying state and eventual exact-once convergence.
4. Fresh context: complete the disposal procedure, close only the named session, start a second globally unique session with a distinct fresh ephemeral profile, and before its explicit login flow prove zero prior IndexedDB databases/entries, session/local storage entries, cookies, CacheStorage names, and service workers, a null prior-context nonce, and browser-cache prior state absent by creation of the new never-used ephemeral profile. Record the second session/profile/context identities and exact explicit-headless real-Chrome launch tuple in `fresh-context-observation.json`; the standard evidence descriptor and environment record the same identities. Then exercise only the required login flow and repeat identity/stale-context proof and the critical visible workflow. The parsed artifact binds context generation 2, one document, one dev-ticket request, capture completeness, semantic observation, the typed reconstruction record, fresh nonce, first-context disposal, second-context disposal/session closure, and absence of stale first-context state. Fresh empty-context evidence is separate from ordinary reload and is not hard-reload proof. Session/profile/context identities must be unique across lifecycle attempts.

Each reconstruction step is a typed record, not the string `pass`: it names a referenced standard evidence artifact, status, `missing_count`, `duplicate_count`, `order_errors`, and optional mapped baseline ID. Navigation, expected traffic, semantic-state observation, and capture completeness are capability facts and never imply that product reconstruction passed. A passing product result requires all three counts to be zero and no baseline; a divergent product result requires a nonzero measured discrepancy and a baseline present in the active manifest. Product failures may remain only as one of the ten finite mapped baselines; infrastructure, leakage, wrong-head, stale-context, headed, non-real-Chromium, or evidence-completeness failures always stop P03.
...
Retain the following under `tmp/gg/gooseweb-migration/P03/<sha7>/attempt-<n>/`: manifest copy/hash, standard evidence-run descriptor, `p03-browser-evidence.json`, parsed `fresh-context-observation.json`, redacted environment/lease/stack tuple, three screenshots, complete console/network/WebSocket captures, redacted runtime/Tower/store state, checks, procedure-overhead measurement, report, and typed outcome. Validate both P03 JSON artifacts against their source-controlled schemas and semantic/cross-artifact validators.
